...
$sform = new XoopsThemeForm(_MD_SBILLING_SEARCH , "form", XOOPS_URL."/modules/smartbilling/search.php", 'GET');
$sform->setExtra('enctype="multipart/form-data"');

// Category
$smartpermissions_handler = new SmartobjectPermissionHandler($smartbilling_category_handler);
...
$categoriesArray = $smartbilling_category_handler->getList($criteria);
unset($criteria);
$newArray = array();
...
$category_select = new XoopsFormRadio(_MD_SBILLING_CATEGORY, 'categoryid', $categoryid);
$category_select->setDescription(_MD_SBILLING_CATEGORY_DSC);
$category_select->addOption(0, _MD_SBILLING_SEARCH_ALL);
$category_select->addOptionArray($categoriesArray);
$category_select->setExtra("onchange='submit()'");
$sform->addElement($category_select);

// ITEM TITLE
$title_text = new XoopsFormText(_MD_SBILLING_TITLE, 'title', 30, 255, $title);
$title_text->setDescription(_MD_SBILLING_TITLE_SEARCH_DSC);
$sform->addElement($title_text);

// ITEM TITLE
$desc_text = new XoopsFormText(_MD_SBILLING_DESCRIPTION, 'desc', 30, 255, $desc);
$desc_text->setDescription(_MD_SBILLING_DESC_SEARCH_DSC);
...
$attObjs = $smartbilling_category_attribut_handler->getObjects($criteria);
...
foreach($attObjs as $att){
	if($att->getVar('display')){
		...
		switch($att_type){
			...
			$radio_box->addOption('Any', _MD_SBILLING_ANY);
			...
			$select_box->addOption('Any', _MD_SBILLING_ANY);
			...
			$yesno_box->addOptionArray(array(0 => _MD_SBILLING_NO, 1 => _MD_SBILLING_YES, 'Any' => _MD_SBILLING_ANY));
...
//-----------------------------------
$search_type_yn = new XoopsFormRadioYN(_MD_SBILLING_SEARCH_ANDOR, 'andor', 1);
$search_type_yn->setDescription(_MD_SBILLING_SEARCH_ANDOR_DSC);
...
$butt_search = new XoopsFormButton('', 'post', _MD_SBILLING_SEARCH, 'submit');
